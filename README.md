...
기존 플랫폼들은 리전 간 비용 차이와 네트워크 지연을 고려하지 않는다. 본 플랫폼은 클라우드 운용 비용과 참여자 간 지연 시간을 종합 분석하여 최적의 집계자 배포 위치를 추천함으로써 비용 절감과 학습 속도 향상을 동시에 달성한다.

**(3) 계층적 보안 아키텍처**

기존 플랫폼은 모든 프로세스를 단일 클라우드에서 처리하여 민감 데이터가 퍼블릭 클라우드에 노출된다. 본 플랫폼은 프라이빗 클라우드의 참여자에 원본 데이터를 격리하고, 퍼블릭 클라우드의 집계자는 모델 파라미터만 처리하도록 계층화하여 데이터 유출 위험을 최소화한다.
...
기존 플랫폼은 클라우드별 분산된 관리 도구를 사용해야 한다. 본 플랫폼은 웹 기반 통합 인터페이스로 참여자 선택부터 모델 배포, 실시간 모니터링, 결과 분석까지 전체 프로세스를 단일 화면에서 관리할 수 있어 운영 편의성을 크게 향상시킨다.
