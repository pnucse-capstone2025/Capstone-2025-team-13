...
기존 클라우드 기반 연합학습의 경우, **단일 클라우드 플랫폼**에 한정되어 연합학습을 수행한다.
이로인해 **리전별 비용 정책 차이와 네트워크 지연(latency)** 을 고려하지 못한다.
학습 참여자가 물리적으로 멀리 떨어져 있는 경우, 모델 파라미터 전송에 지연이 발생하여 **학습 속도가 저하**된다.
또한 비용이 높은 리전에 집계자가 배포되면 **클라우드 비용이 불필요하게 증가**한다.
